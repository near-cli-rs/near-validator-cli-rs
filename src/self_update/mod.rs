#[cfg(windows)]
const BIN_NAME: &str = "near-validator.exe";
#[cfg(not(windows))]
const BIN_NAME: &str = "near-validator";
...
impl SelfUpdateCommandContext {
    pub fn from_previous_context(
        ...
        let status = self_update::backends::github::Update::configure()
            ...
            .update()
            .wrap_err("Failed to update near-validator")?;
        if let self_update::Status::Updated(release) = status {
            println!(
                "\n{}{}{}\n",
                "Welcome to `near-validator` v".green().bold(),
                release.green().bold(),
                "!".green().bold()
            );
            println!("Report any bugs:\n");
            println!("\thttps://github.com/near-cli-rs/near-validator-cli-rs/issues\n");
            println!("What's new:\n");
            println!(
                "\t{}{}\n",
                "https://github.com/near-cli-rs/near-validator-cli-rs/releases/tag/v"
                    .truecolor(0, 160, 150),
